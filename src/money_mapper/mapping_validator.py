...
VALID_CATEGORIES = {
    ...
    "INCOME", "MEDICAL", "PERSONAL_SERVICES", "PERSONAL_TAX", "RENT_AND_UTILITIES",
    "SHOPPING", "TRANSFER", "TRANSPORTATION", "UNCATEGORIZED", "UNKNOWN"
}
...
VALID_SUBCATEGORIES = {
    ...
    "TRANSPORTATION_GAS", "TRANSPORTATION_PARKING", "TRANSPORTATION_PUBLIC_TRANSIT",
    "TRANSPORTATION_TAXI", "TRANSPORTATION_TOLLS", "TRANSPORTATION_OTHER_TRANSPORTATION",
    "SHOPPING_GENERAL", "SHOPPING_ONLINE", "SHOPPING_DRUGSTORE", "SHOPPING_ELECTRONICS",
...
def validate_single_mapping(pattern: str, mapping: Any) -> list[str]:
    ...
    if "scope" in mapping:
        ...
        if scope and scope not in VALID_SCOPES:
            issues.append(f"Mapping '{pattern}' has invalid scope: {scope} (must be 'public' or 'private')")
...
def validate_mapping_structure(mappings: Any) -> list[str]:
    ...
    for section, section_mappings in mappings.items():
        ...
        for pattern, mapping in section_mappings.items():
            if not isinstance(mapping, dict):
                issues.append(f"Mapping for pattern '{pattern}' in section '{section}' must be a dictionary")
...
def validate_categories_consistency(
    ...
    category_taxonomy: dict[str, set[str]] | None = None
) -> list[str]:
    """
